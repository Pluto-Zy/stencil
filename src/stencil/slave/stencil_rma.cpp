...
void stencil_iterate_rma(Arguments& args) {
    ...
    // We must explicitly synchronize here to prevent some threads from entering the loop and
    // starting RMA communication while other threads have not yet initialized the corresponding
    // replies.
    athread_ssync_array();

    // Iterations.
    for (unsigned i = 0; i != args.iterations; ++i) {
        athread_rply_t* const send_reply = &replies[i % 2 * 2];
        athread_rply_t* const recv_reply = send_reply + 1;
